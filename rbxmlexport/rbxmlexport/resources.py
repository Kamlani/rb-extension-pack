...
class ReviewRequestResource(WebAPIResource):
    ...
    def build_body_top_diffs(self, request,review):
        ...
        for diff_comment in diff_comments:
            ...
            for single_line in content[0]['lines']:
                chunk = []
                chunk.append(NameArray("linenum",None,[single_line[0]]))
                chunk.append(NameArray("before",None,[single_line[2]]))
                chunk.append(NameArray("after",None,[single_line[5]]))
                counter = counter + 1
    ...
    def get_reply_comment(self, public_replies):
        ...
        for reply in public_replies:
            reply_array = []
            reply_array.append(NameArray(None,"user",[reply.review.get().user.username]))
            reply_array.append(NameArray(None,"comment",[reply.text]))
            replies.append(NameArray(None,None,reply_array))
        return replies
